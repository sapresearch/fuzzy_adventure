...
class TemplateClassifier():
    ...
    q1 = ("SELECT PROGRAMMERS.FULL_NAME FROM TRANSACTIONS INNER JOIN PROGRAMMERS ON TRANSACTIONS.PROGRAMMER_ID = PROGRAMMERS.ID WHERE END_DATE IS NOT NULL GROUP BY PROGRAMMERS.FULL_NAME ORDER BY COUNT(*) DESC LIMIT 1", ())
    ...
    q6 = ("SELECT COUNT(*) FROM TOUCHED INNER JOIN PROGRAMMERS ON TOUCHED.PROGRAMMER_ID=PROGRAMMERS.ID WHERE PROGRAMMERS.NAME='%s'", (str,))
    q7 = ("SELECT AVG(SECONDS_BETWEEN(start_date, END_DATE)) FROM TRANSACTIONS", ())
    q8 = ("SELECT AVG(MPT) FROM TRANSACTIONS", ())
    q9 = ("select count(trans_number) from TRANSACTIONS inner join PROGRAMMERS on TRANSACTIONS.PROGRAMMER_ID = PROGRAMMERS.ID where PROGRAMMERS.FULL_NAME = '%s' and TRANSCATIONS.FLAG_24H is not null", (str,))
